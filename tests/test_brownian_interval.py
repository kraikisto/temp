...
def _setup(device, levy_area_approximation, shape):
    ...
    bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, size=shape, device=device,
...
def test_normality_simple(device, levy_area_approximation):
    ...
    for _ in range(REPS):
        ...
        bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, W=base_W, levy_area_approximation=levy_area_approximation)
...
def test_normality_conditional(device, levy_area_approximation):
    ...
    t0, t1 = 0.0, 1.0
    for _ in range(REPS):
        bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, size=(LARGE_BATCH_SIZE,), device=device,
                                       levy_area_approximation=levy_area_approximation)
...
def test_entropy_determinism(random_order, device, levy_area_approximation, return_U, return_A):
    ...
    bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, size=(), device=device,
