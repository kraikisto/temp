...
class StochasticLorenz(object):
    ...
    def sample(self, x0, ts, noise_std, normalize):
        ...
        xs = torchsdeRealNoise.sdeint(self, x0, ts)
        if normalize:
...
class LatentSDE(nn.Module):
    ...
    def forward(self, xs, ts, noise_std, adjoint=False, method="euler"):
        ...
        if adjoint:
            ...
            zs, log_ratio = torchsdeRealNoise.sdeint_adjoint(
                self, z0, ts, adjoint_params=adjoint_params, dt=1e-2, logqp=True, method=method)
        else:
            zs, log_ratio = torchsdeRealNoise.sdeint(self, z0, ts, dt=1e-2, logqp=True, method=method)
    ...
    def sample(self, batch_size, ts, bm=None):
        ...
        zs = torchsdeRealNoise.sdeint(self, z0, ts, names={'drift': 'h'}, dt=1e-3, bm=bm)
...
def main(
    ...
    bm_vis = torchsdeRealNoise.BrownianInterval(
