...
from torchsdeRealNoise.settings import LEVY_AREA_APPROXIMATIONS, METHODS, NOISE_TYPES, SDE_TYPES
...
def test_against_numerical(sde_cls, sde_type, method, options, adaptive):
    ...
    bm = torchsdeRealNoise.BrownianInterval(
    ...
    def func(inputs, modules):
        ...
        ys = torchsdeRealNoise.sdeint_adjoint(sde, y0, ts, dt=dt, method=method, adjoint_method=adjoint_method,
...
def test_against_sdeint(sde_cls, sde_type, method, options, dt, rtol, atol, len_ts):
    ...
    ys_true = torchsdeRealNoise.sdeint(sde, y0, ts, dt=dt, method=method, bm=bm, options=options)
    ...
    ys_test = torchsdeRealNoise.sdeint_adjoint(sde, y0, ts, dt=dt, method=method, bm=bm, adjoint_method=adjoint_method,
...
def test_basic(problem, method, adaptive):
    ...
    problem.zero_grad()
    _, yt = torchsdeRealNoise.sdeint_adjoint(problem, y0, ts, method=method, dt=dt, adaptive=adaptive)
