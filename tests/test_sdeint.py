...
import pytest
import torch
import torchsdeRealNoise
from torchsdeRealNoise.settings import NOISE_TYPES
...
def test_rename_methods(device):
    ...
    ans = torchsdeRealNoise.sdeint(sde, y0, ts, dt=dt, names={'drift': 'forward'})
...
def test_rename_methods_logqp(device):
    ...
    y0 = torch.ones(batch_size, d, device=device)
    ts = torch.linspace(t0, t1, steps=T, device=device)
    ans = torchsdeRealNoise.sdeint(sde, y0, ts, dt=dt, names={'drift': 'forward', 'prior_drift': 'w'}, logqp=True)
...
def test_specialised_functions(sde_type, method):
    ...
    for sde in (fg, f_and_g, g_prod, f_and_g_prod, f_and_g_with_g_prod1, f_and_g_with_g_prod2):
        bm = torchsdeRealNoise.BrownianInterval(t0, t1, (batch_size, m), entropy=45678)
        outs.append(torchsdeRealNoise.sdeint(sde, y0, [t0, t1], dt=dt, bm=bm)[1])
    for o in outs[1:]:
...
def test_sdeint_run_shape_method(sde_cls, use_bm, levy_area_approximation, sde_type, method, adaptive, logqp, device):
    ...
    if use_bm:
        if sde_cls.noise_type == 'scalar':
            ...
            size = (batch_size, m)
        bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, size=size, dtype=dtype, device=device,
...
def _test_sdeint(sde, bm, method, adaptive, logqp, device, should_fail, options):
    ...
    with torch.no_grad():
        try:
            with ctx:
                ans = torchsdeRealNoise.sdeint(sde, y0, ts, bm, method=method, dt=dt, adaptive=adaptive, logqp=logqp,
                                      options=options)
        except ValueError:
            if should_fail:
                return
    ...
    with torch.no_grad():
        with ctx:
            ans = torchsdeRealNoise.sdeint(sde, y0, ts, bm, method=method, dt=dt, adaptive=adaptive, names={'drift': 'h'},
...
def test_reversibility(sde_cls):
    ...
    bm = torchsdeRealNoise.BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_size, brownian_size))
    ys, (f, g, z) = torchsdeRealNoise.sdeint(sde, y0, ts, bm=bm, method='reversible_heun', dt=dt, extra=True)
    backward_ts = -ts.flip(0)
    backward_ys = torchsdeRealNoise.sdeint(minus_sde, ys[-1], backward_ts, bm=torchsdeRealNoise.ReverseBrownian(bm),
