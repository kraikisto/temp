...
def _compare(w0, ts, msg=''):
    bm = torchsdeRealNoise.BrownianPath(t0=t0, w0=w0)
    bp_py_time = _time_query(bm, ts)
    logging.warning(f'{msg} (torchsde.BrownianPath): {bp_py_time:.4f}')

    bm = torchsdeRealNoise.BrownianTree(t0=t0, t1=t1, w0=w0, tol=1e-5)
    bt_py_time = _time_query(bm, ts)
    logging.warning(f'{msg} (torchsde.BrownianTree): {bt_py_time:.4f}')

    bm = torchsdeRealNoise.BrownianInterval(t0=t0, t1=t1, size=w0.shape, dtype=w0.dtype, device=w0.device)
...
class SDE(torchsdeRealNoise.SDEIto):
    def __init__(self):
...
def _time_sdeint(sde, y0, ts, bm):
    now = time.perf_counter()
    with torch.no_grad():
        torchsdeRealNoise.sdeint(sde, y0, ts, bm, method='euler')
    return time.perf_counter() - now
...
def _time_sdeint_bp(sde, y0, ts, bm):
    ...
    ys = torchsdeRealNoise.sdeint(sde, y0, ts, bm, method='euler')
    ys.sum().backward()
    return time.perf_counter() - now
...
def _time_sdeint_adjoint(sde, y0, ts, bm):
    ...
    sde.zero_grad()
    y0 = y0.clone().requires_grad_(True)
    ys = torchsdeRealNoise.sdeint_adjoint(sde, y0, ts, bm, method='euler')
    ys.sum().backward()
    return time.perf_counter() - now
